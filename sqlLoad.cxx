...
int main(int argc, char **argv)
{
    ...
    CalibrationPulseDataSet *pulser_data_set = new CalibrationPulseDataSet(std::string(argv[3]));
    std::string current_file = "";
    int N,M,j,k;
    N = M = j = k = 0;
    
    std::vector<std::string> dataTitles; dataTitles.clear();
    std::string specificTitle="";
    ...
    while (all_files.good() && !all_files.eof())
    {
        ...
        if(current_file.find(std::string(argv[3]))!=std::string::npos)
        {
            ...
            dataTitles.push_back(current_pulse->PrintNameString());
        ...
        {
            std::cout<<"Not loading this file: "<<current_file<<std::endl;
        }
    }
    all_files.close();
